...
const generate = async () => {
  console.info(
    `⏳ Resolving all products in catalog. This could take some time.`
  );

  const productIds = await getAllProductIds();
  const productUrls = productIds.map((id) => `/products/${id}`);
  await writeFile(
    PRODUCTS_PATH_TO_CACHE_FILE,
    JSON.stringify(productUrls),
    "Products"
  );
  console.info(
    `✅ Product URLs written to disk at ${PRODUCTS_PATH_TO_CACHE_FILE}`
  );
  ...
  const categories = await Promise.all(
    productIds.map(async (id) => (await getProduct(id))?.primaryCategoryId)
  );
  ...
  const categoryPaths = Array.from(categoryUrls.keys()).map(
    (id) => `/shop/${id}`
  );

  console.info(
    `✅ ${categoryPaths.length} category paths  generated. Start writing to cache. ⏳`
  );

  await writeFile(
    CATEGORIES_TO_CACHE_FILE,
    JSON.stringify(categoryPaths),
    "Categories"
  );

  console.info(
    `✅ Product category URLs are written to cache at ${CATEGORIES_TO_CACHE_FILE}`
  );
};
